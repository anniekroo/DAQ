...
def read_and_display_data(hat, num_channels):
    ...
    f=open("test_out.txt",'ab')#when ctrl-C is used to stop the process, the file is automatically closed.
    #for a production version of the code, it would be cleaner to add a f.close somewhere at the end.
    #I tried to use the 'while' syntax which automatically opens and closes, but the write speed was too slow
    while True:
        read_result = hat.a_in_scan_read(read_request_size, timeout)   #(Origional documentation)   
        #read_result.data is a list with the measured value for each channel
        str_data=str(read_result.data)[1:-1] #removes square brackets from printed output
        f.write(str_data) 
        f.write("\n")
